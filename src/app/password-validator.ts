  ...
  validatePassword(control: AbstractControl): Promise<{ [key: string]: boolean }> {
    ...
    return new Promise((resolve, reject) => {
      this.timeout = setTimeout(() => {
        ...
        this.http.get("https://chiali-backend.herokuapp.com/user/"+control.value+"/"+control.root.get('username').value,{headers:new HttpHeaders({'Authorization':this.authSer.jwtToken})})
          .subscribe(flag => {
              if (flag) {
